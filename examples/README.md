...
Small, copy-paste-able bundles for the common deployment shapes. Namespaces,
...
| File | Shows |
| --- | --- |
| [`fleet-namespace.yaml`](fleet-namespace.yaml) | The v0.19 fleet shape ([deployment shapes](../docs/install/deployment-shapes.md)): in-namespace `Project` + per-key `ProjectRole`s + `OIDCApp` and a `MachineUser` role grant, all wired via `projectRef` — no ScopeMaps. |
